                                        ...
                                        unsigned int featureCount)
{
  ...
  double W, sigma2Hat, delta;
  double gammaHat;
  double A, B, C;
  double cumInvRisk, sumCumInvRisk, sumCumInvRiskSq;
  unsigned int i, k;
  unsigned int leftSize, rightSize, D;
  W = 0.0;
  sigma2Hat = 0.0;
  delta = 0.0;
  ...
  if (leftSize < 2 || rightSize < 2 || D < 2) {
    return 0.0;
}
  
  gammaHat = ((double) leftSize) / ((double) n);

  if (gammaHat <= EPS_CUSTOM || gammaHat >= 1.0 - EPS_CUSTOM) {
    return 0.0;
}
  ...
  for (k = 1; k <= eventTimeSize; k++) {
    double riskAll, riskLeft, riskRight, dLeft, dRight;
  
    riskAll  = (double) nodeParentAtRisk[k];
    riskLeft = (double) nodeLeftAtRisk[k];
    riskRight= riskAll - riskLeft;
    dLeft    = (double) nodeLeftEvent[k];
    dRight   = (double) nodeParentEvent[k] - dLeft;
  
    if (riskAll > EPS_CUSTOM) {
      W += (riskRight * dLeft - riskLeft * dRight) / riskAll;
      cumInvRisk += 1.0 / riskAll;
      sumCumInvRisk += cumInvRisk;
      sumCumInvRiskSq += cumInvRisk * cumInvRisk;
   }
  }
  
  A = 1.0 + ((1.0 - CUSTOM_P) * (1.0 - CUSTOM_Q) / CUSTOM_Q);
  B = ((1.0 - CUSTOM_P) / CUSTOM_P) - (((1.0 - CUSTOM_P) * (1.0 - CUSTOM_Q)) / CUSTOM_Q);
  C = -((1.0 - CUSTOM_P) / CUSTOM_P) + (((1.0 - CUSTOM_P) * (1.0 - CUSTOM_Q)) / CUSTOM_Q);

  sigma2Hat =
    gammaHat * (1.0 - gammaHat) *
    (
      A * (((double) D) / ((double) n)) +
          2.0 * B * (sumCumInvRisk / ((double) n)) +
          C * (sumCumInvRiskSq / ((double) n))
    );
