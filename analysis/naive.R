...
for (m in 1:nsim){
  ...
  start_time <- Sys.time()
  number = tune(Surv(time, status) ~ ., data = cc, ntree=500)
  srf.fit = rfsrc(Surv(time, status) ~ ., data = cc, ntree=500,mtry=number$optimal[1], nodesize=number$optimal[2])
...
write.csv(result,file=paste0(path.results, 'cc_uw.csv'), row.names = F)
