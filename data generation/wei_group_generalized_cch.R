...
path.test <- "/nas/longleaf/home/haolin/dissertation2/G_tree2/data/"
path.cc <- "/nas/longleaf/home/haolin/dissertation2/G_tree2/data/"
...
for (j in 1:nsim){
  ...
  true.lin.pred <- 0.8*(X[,p.full]>0) -2*(X[,p.full+1]<0) + 0.5*(X[,p.full+2]>0)*(X[,p.full+4]>0)*(X[,p.full+5]>0)+1.6
  v <- runif(n=2*n)
