...
path.test <- "/nas/longleaf/home/haolin/dissertation2/G_non2_hi/data/"
path.cc <- "/nas/longleaf/home/haolin/dissertation2/G_non2_hi/data/"
...
for (j in 1:nsim){
  ...
  true.lin.pred <- X %*% beta + 0.3*X[,p.full-1]*X[,p.full+1] + 0.2*X[,p.full+1]^2 + 0.1*X[,p.full-1]*X[,p.full+4]*X[,p.full+7]+2.5
  v <- runif(n=2*n)
