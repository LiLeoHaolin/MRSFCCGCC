...
path.test <- "/nas/longleaf/home/haolin/dissertation2/tree2_hi/data/"
path.cc <- "/nas/longleaf/home/haolin/dissertation2/tree2_hi/data/"
...
for (j in 1:nsim){
  ...
  true.lin.pred <- 0.8*(X[,p.full]>0) -2*(X[,p.full+1]<0) + 0.5*(X[,p.full+2]>0)*(X[,p.full+4]>0)*(X[,p.full+5]>0)+1.5
  v <- runif(n=2*n)
