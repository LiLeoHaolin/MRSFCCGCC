...
path.test <- "/nas/longleaf/home/haolin/dissertation2/non2_hi/data/"
path.cc <- "/nas/longleaf/home/haolin/dissertation2/non2_hi/data/"
...
for (j in 1:nsim){
  ...
  true.lin.pred <- X %*% beta + 0.3*X[,p.full-1]*X[,p.full+1] + 0.2*X[,p.full+1]^2 + 0.1*X[,p.full-1]*X[,p.full+4]*X[,p.full+7]+0.9 #+ 0.3*X[,1]*X[,3] + 0.2*X[,3]^2 + 0.2*X[,1]*X[,6]*X[,9]
  v <- runif(n=2*n)
