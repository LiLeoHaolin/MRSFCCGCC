...
path.test <- "/nas/longleaf/home/haolin/dissertation2/G_round4_hi/data/"
path.cc <- "/nas/longleaf/home/haolin/dissertation2/G_round4_hi/data/"
...
for (j in 1:nsim){
  ...
  true.lin.pred <- X %*% beta +2.7 #+ 0.3*X[,1]*X[,3] + 0.2*X[,3]^2 + 0.2*X[,1]*X[,6]*X[,9]
  v <- runif(n=2*n)
